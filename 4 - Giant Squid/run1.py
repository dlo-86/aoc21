...
def has_a_board_a_bingo(bingo_boards: List[BingoBoard]):
    for bingo_board in bingo_boards:
        if bingo_board.check_lines():
            return bingo_board
    return None
...
def play(bingo_numbers: List[int], bingo_boards: List[BingoBoard]):
    for number in bingo_numbers:
        ...
        bingo_board = has_a_board_a_bingo(bingo_boards)
        if bingo_board:
            print(number, bingo_board.get_sum_of_unmarked_numbers())
            return number * bingo_board.get_sum_of_unmarked_numbers()
    # result: last number * unmarked number
